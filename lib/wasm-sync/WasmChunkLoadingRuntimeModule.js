			...
			Template.indent([
				...
				Template.indent([
					...
					Template.indent([
						...
						"var mimeType = null;",
						`var req = ${this.generateLoadBinaryCode(wasmModuleSrcPath)}.then(function(response) {`,
						Template.indent([
						...
						this.supportsStreaming
							? Template.asString([
									...
									Template.indent([
										"var compilePromise = WebAssembly.compileStreaming(req).catch(function(error) {",
										Template.indent([
											'if (mimeType !== "application/wasm") {',
											Template.indent([
												"return req.then(function (x) { return x.arrayBuffer(); }).then(function (bytes) { return WebAssembly.compile(bytes); });"
											]),
											"}"
										]),
										"});",
										"promise = Promise.all([compilePromise, importObject]).then(function(items) {",
										Template.indent([
